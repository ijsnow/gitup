...
func TestAppHelpiocli(t *testing.T) {
	oldiocli := Helpiocli
	defer func() {
		Helpiocli = oldiocli
	}()
	...
	var wasCalled = false
	Helpiocli = func(w io.Writer, template string, data interface{}) {
		wasCalled = true
	}
	...
	if wasCalled == false {
		t.Errorf("Help iocli expected to be called, but was not")
	}
}

func TestApp_Versioniocli(t *testing.T) {
	oldiocli := Versioniocli
	defer func() {
		Versioniocli = oldiocli
	}()

	var wasCalled = false
	Versioniocli = func(c *Context) {
		wasCalled = true
	}
	...
	if wasCalled == false {
		t.Errorf("Version iocli expected to be called, but was not")
	}
}
